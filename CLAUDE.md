...
- `data/ingredients/<id>.yaml` — the **locale-neutral core**, one per ingredient: per-100g nutrition + `density_g_per_ml` + FDC id, and nothing locale-specific (a library asset — the canonical locale gets no inline privilege). Each supported locale owns `data/ingredients/<locale>/<id>.yaml` (names/aliases/aisle + optional `availability` market guidance authored in that market's language: `notes` with an `important` flag that pins to the Shop row, `brands`; no gate on availability). Assembled at load; every locale needs its file with a name (aisle is all-or-nothing across locales) or the build fails.
