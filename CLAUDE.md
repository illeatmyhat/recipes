...
- `data/ingredients/*.yaml` — one file per ingredient: per-100g nutrition + `density_g_per_ml` + inline en-US/ja-JP names/aliases + per-locale `aisle` + optional per-locale `availability` (market guidance authored in that market's language: `notes` with an `important` flag that pins to the Shop row, `brands`; no completeness gate). Other locales are **overlay folders** mirroring the filenames (`data/ingredients/zh-CN/<id>.yaml`: names/aliases/aisle/availability), merged at load; every supported locale needs a name + aisle or the build fails.
