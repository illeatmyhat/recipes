/**
 * Step metadata extraction (v3) — the structured channel from the MDX body to
 * the bundle.
 *
...
export function extractSteps(body: string): ExtractedStep[] {
  ...
  for (const m of body.matchAll(STEP)) {
    ...
    const id = attrs.id;
    if (id === undefined || id === '') {
      throw new Error(`v3 steps: a <Step> is missing its id (near "${inner.slice(0, 40).trim()}…").`);
    }
    if (seen.has(id)) {
      throw new Error(`v3 steps: duplicate step id "${id}".`);
    }
    seen.add(id);
    ...
    for (const r of inner.matchAll(REF)) {
      ...
      if (role === undefined || role === '') {
        throw new Error(`v3 steps: step "${id}" has a <Ref> with no of="role".`);
      }
